...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    z_matrix: np.ndarray = feature_matrix * theta
    z_matrix = z_matrix.sum(axis=1)  # x_i and theta dot product
    z_matrix = z_matrix.astype(float)
    z_matrix += theta_0  # x_i * theta + theta_0
    z_matrix *= labels  # y_i * (x_i * theta + theta_0)
    hinge_loss_matrix = np.where(z_matrix >= 1, 0, 1 - z_matrix)
    return sum(hinge_loss_matrix) / len(hinge_loss_matrix)
...
def pegasos_single_step_update(feature_vector, label, L, eta, theta, theta_0):
    ...
    if label * (np.dot(feature_vector, theta) + theta_0) <= 1:
        updated_theta = theta * (1 - eta * L) + eta * feature_vector * label
        updated_theta_0 = theta_0 + eta * label
    else:
        updated_theta = theta * (1 - eta * L)
...
def classify(feature_matrix, theta, theta_0):
    ...
    result = np.where(abs(result) < epsilon, 0, result)
